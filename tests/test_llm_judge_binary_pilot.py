"""Binary TRAIN pilot provenance, audit alignment, budgets and resume tests."""
...
from post_thesis.llm_judge import run_binary_pilot as pilot
...
class BudgetTests(unittest.TestCase):
    ...
    def test_committed_plan_records_operator_supplied_checksums_and_budget(self):
        ...
        self.assertEqual(plan["audit_sha256"], "55c8b27a10cf0f26c6dfcb35d6882d2c9b400a8fa90bb20689ab7746348f181b")
        ...
        self.assertEqual(plan["audited_input_tokens"] + plan["max_output_tokens_total"], 66874)
...
class PilotExecutionTests(unittest.IsolatedAsyncioTestCase):
    ...
    def setup_inputs(self, backend):
        ...
        counts = {ex.sample_id: {"status": "ok", "input_tokens": 100,
                  "request_key": binary_request(ex.item, backend.config).key} for ex in examples}
    ...
    async def test_fifty_bounded_requests_and_cache_replay_without_network(self):
        async with self.backend() as backend:
            ...
            self.assertEqual(result["report"]["examples_judged"], 50)
    ...
    async def test_chunked_resume_accumulates_time_and_skips_successes(self):
        async with self.backend() as backend:
            ...
            self.assertEqual(second["report"]["examples_judged"], 2)
    ...
    async def test_no_generation_when_audited_count_changes(self):
        async with self.backend() as backend:
            ...
            summary = json.loads((self.run_dir / "binary_pilot_summary.json").read_text())
    ...
    async def test_failed_attempt_is_not_retried(self):
        async with self.backend() as backend:
            ...
            self.assertEqual(second["report"]["examples_judged"], 1)
    ...
    async def test_binary_schema_identity_and_run_namespace_are_enforced(self):
        async with self.backend() as backend:
            self.setup_inputs(backend)
    ...
    async def test_report_keeps_string_verdicts_and_never_probabilities(self):
        async with self.backend() as backend:
            ...
            self.assertEqual(result["report"]["predictions"][0]["verdict"], "unsupported")
