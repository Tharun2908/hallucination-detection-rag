...
from post_thesis.llm_judge.evidence_schema_v2 import evidence_request_v2, EVIDENCE_SCHEMA_V2_JSON
...
class SchemaV2ExecutionTests(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory()
        self.addCleanup(self.temp.cleanup)
        self.root = Path(self.temp.name)
        self.directory = run_directory(diagnostic.load_plan('v2')['run_id'], self.root)
        self.calls = []
        self.tokens = 0
        ...
        self.server = {'study_stage': 'post_thesis', 'kind': 'serving_session', 'status': 'started',
                       'code_revision': 'v2-test', 'profile': PROFILE, 'gpu': {'name': 'NVIDIA H200'},
                       'command': server_command(profile_name='evidence-v1'),
                       'package_versions': diagnostic.native_observation()['versions']}
    ...
    async def execute(self, backend, **kwargs):
        return await diagnostic.execute(backend=backend, revision='v2-test', server_record=self.server,
                                        artifact_root=self.root, schema_version='v2', **kwargs)

    async def test_v2_wire_same_messages_new_schema_separate_cache_and_bounded_replay(self):
        async with self.backend() as backend:
            ...
            for wire, example in zip(wires, examples()):
                ...
                new = evidence_request_v2(example.item, backend.config)
                ...
                self.assertEqual(payload['response_format']['json_schema']['schema'], json.loads(EVIDENCE_SCHEMA_V2_JSON))
    ...
    async def test_changed_compatibility_or_budget_cannot_enable_calls(self):
        ...
        async with self.backend() as backend:
            for key, value in [('native_observation_sha256', 'changed'), ('client_budget_seconds', 600),
                ...
                altered = original('v2'); altered[key] = value
    ...
    async def test_prior_v1_journal_cannot_be_reused_as_v2(self):
        import shutil
        async with self.backend() as backend:
            await diagnostic.execute(backend=backend, revision='v2-test', artifact_root=self.root,
