"""Evidence TRAIN pilot provenance, audit alignment, budgets and resume tests."""
...
from post_thesis.llm_judge.evidence_contract import EVIDENCE_PROMPT
...
from post_thesis.llm_judge import run_evidence_pilot as pilot
...
class BudgetTests(unittest.TestCase):
    ...
    def test_committed_plan_records_operator_supplied_checksums_and_budget(self):
        ...
        self.assertEqual(plan["audit_sha256"], "d08eede188c9308e2a77cc9e205e324c0cace58b293b05afa965e24a02f48939")
        ...
        self.assertEqual(plan["audited_input_tokens"] + plan["max_output_tokens_total"], 103174)
...
class PilotExecutionTests(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        ...
        self.slow = False
        self.response_text = json.dumps(dict(verdict="supported", issue_type="none", answer_quote=None, context_quote=None, explanation=None))
    ...
    def backend(self):
        ...
        return VLLMBackend(profile_name="evidence-v1", transport=httpx.MockTransport(handler))

    def setup_inputs(self, backend):
        self.plan = pilot.load_plan(self.PROMPT.version)
        ...
        counts = {ex.sample_id: {"status": "ok", "input_tokens": 100,
                  "request_key": self.REQUEST_FACTORY(ex.item, backend.config).key} for ex in examples}
        state = {"identity": {"study_stage": "post_thesis", "audit_version": AUDIT_VERSION,
                 ...
                 "request_contract": self.plan["request_contract"],
                 "response_schema_sha256": self.plan["response_schema_sha256"],
                 "serialized_schema_sha256": self.plan["serialized_schema_sha256"],
    ...
    async def test_fifty_bounded_requests_and_cache_replay_without_network(self):
        async with self.backend() as backend:
            ...
            self.assertEqual(result["report"]["examples_valid"], 50)
            self.assertEqual(result["new_attempts_this_invocation"], 50)
            self.assertEqual(result["report"]["response_order_summary"]["matches"], 50)
            ...
            for wire in generations:
                ...
                self.assertEqual(payload["response_format"]["json_schema"]["name"], "faithfulness_evidence_verdict_v2")
            ...
            self.assertEqual(cached["report"]["response_order_summary"]["matches"], 50)
    ...
    async def test_chunked_resume_accumulates_time_and_skips_successes(self):
        async with self.backend() as backend:
            ...
            self.assertEqual(second["report"]["examples_valid"], 2)
    ...
    async def test_no_generation_when_audited_count_changes(self):
        async with self.backend() as backend:
            ...
            self.assertEqual(result["status"], "halted_on_serving_or_alignment_error")
            self.assertNotIn("/v1/chat/completions", [r.url.path for r in self.calls])
            summary = json.loads((self.run_dir / "evidence_pilot_summary.json").read_text())
            ...
            self.assertEqual(resumed["status"], "blocked_by_retained_halt")
            self.assertEqual(before, len(self.calls))
    ...
    async def test_failed_attempt_is_not_retried(self):
        async with self.backend() as backend:
            ...
            self.assertEqual(second["report"]["examples_valid"], 1)
    ...
    async def test_evidence_schema_identity_and_run_namespace_are_enforced(self):
        async with self.backend() as backend:
            self.setup_inputs(backend)
            self.audited["audit"]["identity"].pop("request_contract")
            self.audited["audit_sha256"] = content_hash(self.audited["audit"])
            self.plan["audit_sha256"] = self.audited["audit_sha256"]
            with self.assertRaises(RunConflict):
                await self.execute(backend)
            self.assertEqual(self.calls, [])
            self.assertFalse(self.run_dir.exists())
            self.setup_inputs(backend)
            self.plan["run_id"] = "qwen3-ragtruth-train-pilot-50-v2"
            with self.assertRaises(RunConflict):
                await self.execute(backend)
            self.assertEqual(self.calls, [])
            self.assertFalse(self.run_dir.exists())

    async def test_report_keeps_string_verdicts_and_never_probabilities(self):
        async with self.backend() as backend:
            self.setup_inputs(backend)
            result = await self.execute(backend, max_new_attempts=1)
            self.assertEqual(result["coverage"], 1 / 50)
            self.assertEqual(result["report"]["predictions"][0]["verdict"], "supported")
            self.assertNotIn("unsupported_probability", json.dumps(result))
            self.assertTrue(all(p["verdict"] is None for p in result["report"]["predictions"][1:]))



    async def test_schema_order_changes_rejected_even_with_rehashed_audit(self):
        async with self.backend() as backend:
            self.setup_inputs(backend)
            self.audited["audit"]["identity"]["response_schema_json"] = json.dumps(json.loads(EVIDENCE_SCHEMA_V3_JSON), sort_keys=True)
            self.audited["audit_sha256"] = content_hash(self.audited["audit"])
            self.plan["audit_sha256"] = self.audited["audit_sha256"]
    ...
    async def test_valid_wrong_order_is_reported_separately(self):
        async with self.backend() as backend:
            self.setup_inputs(backend)
            self.response_text = json.dumps(json.loads(self.response_text), sort_keys=True)
            result = await self.execute(backend, max_new_attempts=1)
            self.assertEqual(result["report"]["examples_valid"], 1)
            self.assertEqual(result["report"]["response_order_summary"]["mismatches"], 1)

    async def test_invalid_evidence_verdict_is_not_salvaged(self):
        async with self.backend() as backend:
            self.setup_inputs(backend)
            self.response_text = json.dumps(dict(verdict="unsupported", issue_type="contradiction",
                answer_quote="FABRICATED ANSWER QUOTE", context_quote="FABRICATED CONTEXT QUOTE", explanation="A mismatch."))
            result = await self.execute(backend, max_new_attempts=1)
            row = result["report"]["predictions"][0]
            self.assertEqual(result["report"]["examples_terminal_failure"], 1)
    ...
    async def test_schema_http_error_halts_and_stays_blocked(self):
        async with self.backend() as backend:
            self.setup_inputs(backend)
            self.completion_status = 400
            result = await self.execute(backend)
            self.assertEqual(result["error_code"], "http_400")
            self.assertEqual(result["report"]["attempts_total"], 1)
            before = len(self.calls)
            resumed = await self.execute(backend)
            self.assertEqual(resumed["status"], "blocked_by_retained_halt")
            self.assertEqual(before, len(self.calls))

    async def test_changed_environment_blocks_before_http(self):
        async with self.backend() as backend:
            self.setup_inputs(backend)
            self.server["package_versions"]["xgrammar"] = "different"
            with self.assertRaises(RunConflict):
                await self.execute(backend)
            self.assertEqual(self.calls, [])


class PromptV2ExecutionTests(PilotExecutionTests):
    PROMPT = EVIDENCE_PROMPT_V2
    REQUEST_FACTORY = staticmethod(evidence_request_prompt_v2)

    async def test_v1_audit_rejected_even_when_outer_checksum_is_recomputed(self):
        async with self.backend() as backend:
            self.setup_inputs(backend)
            state = self.audited["audit"]
            state["identity"]["prompt"] = {"version": EVIDENCE_PROMPT.version, "sha256": EVIDENCE_PROMPT.sha256}
            self.audited["audit_sha256"] = content_hash(state)
            self.plan["audit_sha256"] = self.audited["audit_sha256"]
            with self.assertRaisesRegex(RunConflict, "identity"):
                await self.execute(backend)
            self.assertEqual(self.calls, [])
    ...
    async def test_v1_request_keys_cannot_be_used_with_v2_audit_identity(self):
        async with self.backend() as backend:
            self.setup_inputs(backend)
            state = self.audited["audit"]
            for example in pilot_examples(self.bundle):
                state["counts"][example.sample_id]["request_key"] = evidence_request_v3(example.item, backend.config).key
            self.audited["audit_sha256"] = content_hash(state)
            self.plan["audit_sha256"] = self.audited["audit_sha256"]
            with self.assertRaisesRegex(RunConflict, "misaligned"):
                await self.execute(backend)
            self.assertEqual(self.calls, [])

    async def test_cannot_spend_v2_plan_in_v1_namespace(self):
        async with self.backend() as backend:
            self.setup_inputs(backend)
            self.plan["run_id"] = pilot.load_plan()["run_id"]
            with self.assertRaisesRegex(RunConflict, "namespace"):
                await self.execute(backend)
            self.assertEqual(self.calls, [])
            self.assertFalse(self.run_dir.exists())

    def test_v2_plan_pins_observed_audit_and_same_compute_limits(self):
        plan = pilot.load_plan(self.PROMPT.version)
        self.assertEqual(plan["audit_sha256"], "06569291a3b0b620884ad1cdd213ffa83febc718c827e5a388bb133a888f92d6")
        self.assertEqual(plan["audit_code_revision"], "6137fb47b5aa65ace9eb5de78a8bea52202850bc")
        self.assertEqual(plan["audited_input_tokens"] + plan["max_output_tokens_total"], 103324)
        self.assertEqual(plan["audited_max_input_tokens"] + plan["max_output_tokens_per_example"], 3841)
        old = pilot.load_plan()
        for key in ("pilot_manifest_sha256", "profile_sha256", "serialized_schema_sha256",
                    "client_budget_seconds_across_resumes", "max_attempts_per_input",
                    "max_output_tokens_per_example", "max_output_tokens_total"):
