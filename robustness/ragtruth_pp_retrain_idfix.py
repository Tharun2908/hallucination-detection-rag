...
def load_and_match():
    ...
    msgs["meta_parsed"] = msgs["meta"].apply(json.loads)
    ...
    print("\nLoading original RAGTruth test...")
    ...
    # Canonical mapping:
    #
    # RAGTruth++ meta.original_id -> RAGTruth id
    #
    # The integrity audit confirmed:
    #   - all 17,790 RAGTruth IDs are unique
    #   - all 408 RAGTruth++ responses resolve uniquely
    #   - all 408 originate from the original RAGTruth test split
    #   - response text and generator model agree for every match
    # ---------------------------------------------------------
    ...
    for pp_row_idx, row in assistant.iterrows():
        ...
        if "original_id" not in meta:
            raise RuntimeError(
                f"Missing original_id for RAGTruth++ row {pp_row_idx}"
            )
        ...
        if original_id not in id_to_info:
            raise RuntimeError(
                f"RAGTruth++ original_id {original_id} "
                "not found in RAGTruth test"
            )

        idx, rt_ex = id_to_info[
            original_id
        ]
        ...
        if content != str(rt_ex["output"]):
            raise RuntimeError(
                f"Response mismatch for original_id={original_id}"
            )

        if str(meta.get("model")) != str(rt_ex.get("model")):
            raise RuntimeError(
                f"Model mismatch for original_id={original_id}"
            )
        ...
        matched.append({
            ...
            "label_orig":
                int(is_hallucinated(rt_ex)),
        })

    if len(matched) != 408:
        raise RuntimeError(
            f"Expected 408 matches, got {len(matched)}"
        )
    ...
    n_flipped = sum(
        1
        for m in matched
        if m["label_pp"] != m["label_orig"]
    )

    print(
        f"\nMatched exactly by original_id: "
        f"{len(matched)} / {len(assistant)}"
    )

    print(
        f"  RAGTruth++ pos rate: "
        f"{n_pp_pos / len(matched):.3f} "
        f"({n_pp_pos}/{len(matched)})"
    )

    print(
        f"  Original pos rate:   "
        f"{n_orig_pos / len(matched):.3f} "
        f"({n_orig_pos}/{len(matched)})"
    )

    print(
        f"  Flipped labels:      "
        f"{n_flipped} "
        f"({n_flipped / len(matched):.1%})"
    )
    ...
    print(
        "    original RAGTruth train: 0"
    )

    print(
        f"    original RAGTruth test:  "
        f"{len(matched)}"
    )
