...
The [reserved development scoring runs](results/post_thesis/llm_judge/development_scoring_cluster_20260920.md) now have 600/600 valid scores per arm and zero new attempts on replay. [Offline calibration and threshold fitting](post_thesis/llm_judge/OFFLINE_FITTING.md) is implemented under the unchanged preregistered rules; the [calibration fit and raw threshold are now frozen](results/post_thesis/llm_judge/development_fit_cluster_20260920.md), with identical cached replay. The on-pod freeze check passed. [RAGTruth TEST input alignment and baseline inventory](post_thesis/llm_judge/TEST_ALIGNMENT.md) is now implemented; the cluster manifest and replay now match. All six original baseline caches were located, and a [CPU provenance audit](post_thesis/llm_judge/BASELINE_PROVENANCE.md) is available. Baseline comparison readiness and final evaluation remain pending.

The experiments below were conducted after thesis submission and are not part
of the submitted thesis results.

**Current status: probability and binary TRAIN pilots and both synthetic diagnostics are complete; final benchmark scoring is pending.** The [structured evidence diagnostic](results/post_thesis/llm_judge/evidence_diagnostic_v1_20260919.md) produced 11/14 valid synthetic records and one explanation concern. The [live schema-v2 diagnostic](results/post_thesis/llm_judge/evidence_diagnostic_v2_20260919.md) produced 14/14 valid records with two false positives. The [v3 live result](results/post_thesis/llm_judge/evidence_diagnostic_v3_20260919.md) has 14/14 valid, correctly ordered outputs and 13/14 matching verdicts. The [original TRAIN token audit](results/post_thesis/llm_judge/ragtruth_evidence_v3_audit_20260919.md) is complete: all 50 inputs fit. The [evidence TRAIN pilot](results/post_thesis/llm_judge/ragtruth_evidence_pilot_v3_20260919.md) completed with 37/50 valid records, 13 quote-membership failures and semantic errors among valid outputs. The candidate is not ready for a benchmark freeze. The [paired synthetic comparison](results/post_thesis/llm_judge/evidence_prompt_pair_v1_20260919.md) completed with 26/26 valid outputs per prompt: v1 matches 24 expected verdicts and v2 matches 25. V2 fixes one verdict but introduces an incorrect unknown-as-absence rationale; both miss a false context-absence claim. These mixed development findings do not establish overall semantic superiority. The [v2 evidence TRAIN pilot](results/post_thesis/llm_judge/ragtruth_evidence_prompt_v2_pilot_20260920.md) is complete: 43/50 valid outputs, seven quote failures and persistent reasoning errors. On the 37 examples valid in both runs, label agreement changes from 27 to 26; increased coverage does not establish better semantic reasoning. Evidence-prompt tuning is paused. The [continuous-score design](post_thesis/llm_judge/CONTINUOUS_SCORE_PROTOCOL.md) selects raw class-token likelihoods as the next candidate; the [offline contract and tokenizer check](post_thesis/llm_judge/LABEL_SCORE_OFFLINE.md) are implemented. The [cluster tokenizer and installed-source checks](results/post_thesis/llm_judge/label_score_setup_20260920.md) passed. The [synthetic raw-logprob run and cached replay](results/post_thesis/llm_judge/label_score_synthetic_v1_20260920.md) passed transport checks: 30/30 valid scores, zero new replay calls, and two unknown-as-absence errors in each mapping. The [original-50 TRAIN token audit](post_thesis/llm_judge/LABEL_SCORE_PILOT_AUDIT.md) passed with 60,574 input tokens. The [label-score TRAIN pilot and cached replay](results/post_thesis/llm_judge/label_score_train_pilot_v1_20260920.md) completed with 50/50 valid scores and zero replay calls. Development AUROC is 0.7051 and emitted-label F1 is 0.5455, below the earlier binary pilot F1 of 0.6500. Scores remain uncalibrated; an [offline TRAIN native-source/exact-overlap auditor](post_thesis/llm_judge/SOURCE_AUDIT.md) is now implemented. The TRAIN-only cluster check found no additional pilot-linked exclusions. A [native TRAIN–TEST exact-overlap check](post_thesis/llm_judge/CROSS_SPLIT_AUDIT.md) is now implemented; local validation proposes excluding 12 additional candidate rows sharing evidence with native TEST sources. It uses no TEST answers or labels. The [cluster result](results/post_thesis/llm_judge/cross_split_audit_cluster_20260920.md) matches the full report hash. A [development reservation design](post_thesis/llm_judge/DEVELOPMENT_SPLIT_PROTOCOL.md) fixes 100 source components per calibration/threshold role; the [offline selector](post_thesis/llm_judge/DEVELOPMENT_RESERVATION.md) locally reserves 600 examples per role while preserving all 312 exclusions. The [cluster reservation and replay](results/post_thesis/llm_judge/development_reservation_cluster_20260920.md) now match. The [calibration/threshold protocol](post_thesis/llm_judge/CALIBRATION_THRESHOLD_PROTOCOL.md) fixes statistical choices before scoring; no calibration or threshold fitting has occurred. The [reserved-arm CPU token audit](post_thesis/llm_judge/DEVELOPMENT_TOKEN_AUDIT.md) is implemented and locally counts all 1,200 inputs within the model limit. The [cluster audit and replay](results/post_thesis/llm_judge/development_token_audit_cluster_20260920.md) now match. The [bounded scoring plan](post_thesis/llm_judge/DEVELOPMENT_SCORING_RUN.md) permits one request per reserved input in two separate 600-example runs; live scoring and fitting remain pending. Broader disjointness and the final evaluation protocol remain pending. See the [binary pilot findings](results/post_thesis/llm_judge/ragtruth_binary_pilot_20260919.md) and [focused error review](results/post_thesis/llm_judge/ragtruth_binary_error_review_20260919.md). See the [paired findings](results/post_thesis/llm_judge/ragtruth_pilot_v1_v2_20260919.md) and [diagnostic protocol](post_thesis/llm_judge/DIAGNOSTIC.md). See the [v1 pilot findings](results/post_thesis/llm_judge/ragtruth_pilot_v1_20260919.md), [synthetic observations](results/post_thesis/llm_judge/synthetic_smoke_20260919.md), and [v2 development revision](post_thesis/llm_judge/PROMPT_V2.md). The
