"""Judge the frozen 50-example TRAIN pilot with evidence records and a separate budget."""
...
from .judge import BackendError, PROTOCOL_ID
...
from .run_pilot import AuditedBackend, HALT_CODES, _save_ledger, charged_seconds, validate_server
from .serve import code_revision, resource_totals
from .storage import RunConflict, atomic_json, exclusive_run
from .vllm_backend import VLLMBackend

PLAN_PATH = Path(__file__).with_name("configs") / "ragtruth_pilot_50_evidence_v3.json"


EVIDENCE_PILOT_HALT_CODES = HALT_CODES + ("http_400", "http_422", "http_500", "input_too_long", "invalid_token_count")


class EvidenceAuditedBackend(AuditedBackend):
    async def complete(self, request):
        try:
            return await super().complete(request)
        except BackendError as error:
            if error.code in EVIDENCE_PILOT_HALT_CODES:
...
def validate_inputs(bundle, audited, plan, backend):
    ...
    if (plan["study_stage"] != "post_thesis" or plan["protocol_id"] != PROTOCOL_ID
            ...
            or plan.get("profile_name") != "evidence-v1"
            or plan.get("structured_output_backend_policy") != "unchanged_vllm_default_auto"
            or plan["max_output_tokens_per_example"] != 512 or plan["max_output_tokens_total"] != 25600
            or plan["scope"] != "TRAIN_development_evidence_verdicts_not_probabilities"
            ...
            or plan["halt_on_error_codes"] != list(EVIDENCE_PILOT_HALT_CODES)
    ...
    if (identity["study_stage"] != "post_thesis" or identity["audit_version"] != AUDIT_VERSION
            ...
            or identity.get("request_contract") != plan["request_contract"]
            or identity.get("response_schema_sha256") != plan["response_schema_sha256"]
            or identity.get("serialized_schema_sha256") != plan["serialized_schema_sha256"]
            or identity.get("response_schema_json") != EVIDENCE_SCHEMA_V3_JSON
            or identity.get("expected_response_field_order") != plan["expected_response_field_order"]
...
async def execute(bundle, audited, *, plan, backend, revision, artifact_root=None,
    ...
    guarded = EvidenceAuditedBackend(backend, counts)
    ...
    with exclusive_run(control):
        ...
        if report["examples_pending"] and max_new_attempts:
            prior_halt = next((w.get("error_code") for w in ledger["windows"]
                               if w.get("error_code") in EVIDENCE_PILOT_HALT_CODES), None)
            if prior_halt:
                status, error_code = "blocked_by_retained_halt", prior_halt
            elif remaining <= 0:
                ...
                try:
                    ...
                    status = "halted_on_serving_or_alignment_error" if guarded.halt_code else "invocation_completed"
        ...
        spent = charged_seconds(ledger["windows"], total)
        output = {"study_stage": "post_thesis", "kind": "TRAIN_development_evidence_pilot",
                  ...
                  "coverage": report["examples_valid"] / report["examples_total"],
                  "output_contract": "evidence_record_not_probability", "report": report}
        atomic_json(directory / "evidence_pilot_summary.json", output)
...
def main():
    ...
    print("Pilot version:", plan["execution_plan_version"])
    print("Prompt version:", plan["prompt_version"])
    print("Evidence schema version: v3")
    print(f"Valid evidence records: {report['examples_valid']}/{report['examples_total']} TRAIN development examples")
    ...
    print("Private evidence pilot summary:", run_directory(plan["run_id"]) / "evidence_pilot_summary.json")
    print("Post-thesis development only. Evidence verdicts are not probabilities. Manual evidence review pending. No test metrics, threshold fitting or prompt changes.")
    return 0 if (report["examples_valid"] == plan["examples"] and
                 report["response_order_summary"]["matches"] == plan["examples"]) else 1
