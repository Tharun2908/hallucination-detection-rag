"""One bounded binary-only diagnostic invocation; never produces probabilities."""
...
from .diagnostic_cases import CASE_VERSION, case_records, cases_sha256, examples
from .binary_contract import BINARY_PROMPT, BINARY_SCHEMA_JSON
from .binary_runner import run_binary
from .diagnose import LimitedBackend, DIAGNOSTIC_HALT_CODES
...
PLAN_PATH = Path(__file__).with_name("configs") / "binary_diagnostic_v1.json"
...
def comparisons(report):
    ...
    for row in case_records():
        ...
        expected = "unsupported" if row["expected_unsupported"] else "supported"
        matches.append({"sample_id": row["sample_id"], "expected_verdict": expected,
                        "observed_verdict": prediction["verdict"],
                        "matches_expected": None if prediction["verdict"] is None else prediction["verdict"] == expected})
    return matches
...
async def execute(*, backend, revision, server_record=None, artifact_root=None,
    ...
    plan = load_plan()
    cases = examples()
    if (plan["run_id"] != "qwen3-binary-synthetic-diagnostic-v1"
            or plan["study_stage"] != "post_thesis" or plan["case_version"] != CASE_VERSION
            or plan["cases_sha256"] != cases_sha256()
            or plan["prompt_version"] != BINARY_PROMPT.version
            or plan["prompt_sha256"] != BINARY_PROMPT.sha256
            or plan["schema_sha256"] != content_hash(json.loads(BINARY_SCHEMA_JSON))
    ...
    async def run(cap):
        return await run_binary(cases, run_id=plan["run_id"], config=backend.config,
                                backend=guarded, revision=revision, dataset_revision=CASE_VERSION,
                                artifact_root=artifact_root, max_new_attempts=cap,
                                halt_codes=DIAGNOSTIC_HALT_CODES)
    ...
    with exclusive_run(control):
        ...
        summary = {"study_stage": "post_thesis", "kind": "binary_synthetic_diagnostic",
        ...
        atomic_json(directory / "binary_summary.json", summary)
...
def main():
    ...
    print(f"Judged {report['examples_judged']}/10 synthetic examples")
    ...
    print("sample_id,expected_unsupported,verdict")
    for prediction in report["predictions"]:
        sid = prediction["sample_id"]
        print(f"{sid},{expected[sid]},{prediction['verdict']}")
    ...
    return 0 if result["report"]["examples_judged"] == 10 else 1
