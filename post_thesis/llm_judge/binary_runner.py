"""Durable one-attempt binary diagnostic journal, isolated from probability runs."""

from dataclasses import asdict
import math
import time

from .binary_contract import binary_request, parse_verdict
from .judge import BackendError, BackendResponse, TokenUsage
...
def _parsed(response):
    ...
    try:
        return "ok", parse_verdict(response.text), None
    except JudgeParseError as error:
        return "invalid_output", None, error.code


async def binary_once(request, backend):
    ...
    try:
        ...
        status, verdict, error = _parsed(response)
    except BackendError as exc:
        status, verdict, error, usage = "backend_error", None, exc.code, exc.usage
    return {"study_stage": "post_thesis", "request": asdict(request), "status": status,
            "verdict": verdict, "error_code": error,
...
def _validated(records, requests):
    ...
    for row in records:
        ...
        if key not in requests or key in stored or row["ordinal"] != 1:
            raise RunConflict("unexpected or repeated binary diagnostic attempt")
        if row["state"] == "finished":
            ...
            try:
                ...
                if value["status"] == "backend_error":
                    ...
                    if value["response"] is not None or value["verdict"] is not None:
                        raise ValueError
                else:
                    data = dict(value["response"])
                    data["usage"] = TokenUsage(**data["usage"])
                    response = BackendResponse(**data)
                    if response.usage != usage or _parsed(response) != (value["status"], value["verdict"], value["error_code"]):
                        raise ValueError
            except (KeyError, TypeError, ValueError):
                raise RunConflict("cached binary result violates its contract") from None
        stored[key] = row
    return stored
...
def _report(examples, by_id, stored, new_attempts):
    ...
    for example in examples:
        ...
        predictions.append({"sample_id": example.sample_id,
                            ...
                            "verdict": value["verdict"] if value else None,
    ...
    return {"study_stage": "post_thesis", "output_contract": "binary_verdict_not_probability",
            "examples_total": len(examples), "examples_judged": judged,
...
async def run_binary(examples, *, run_id, config, backend, revision, dataset_revision,
    ...
    by_id = {e.sample_id: binary_request(e.item, config) for e in examples}
    requests = {r.key: r for r in by_id.values()}
    if not examples or len(by_id) != len(examples) or len(requests) != len(examples):
        raise ValueError("binary diagnostic requires unique IDs and inputs")
    manifest = {"study_stage": "post_thesis", "kind": "binary_diagnostic", "run_id": run_id,
    ...
    with exclusive_run(directory):
        ...
        try:
            ...
            for example in examples:
                ...
                new += 1
                result = await binary_once(request, backend)
