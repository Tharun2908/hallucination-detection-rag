"""Judge the frozen 50-example TRAIN pilot with binary verdicts and a separate budget."""
...
from .binary_runner import run_binary
...
PLAN_PATH = Path(__file__).with_name("configs") / "ragtruth_pilot_50_binary_v1.json"
...
def validate_inputs(bundle, audited, plan, backend):
    ...
    if (plan["study_stage"] != "post_thesis" or plan["protocol_id"] != PROTOCOL_ID
            or plan["execution_plan_version"] != "ragtruth_pilot_50_binary_v1"
            or plan["run_id"] != "qwen3-ragtruth-train-pilot-50-binary-v1"
            or plan["request_contract"] != "binary-diagnostic-request-v1"
            or plan["response_schema_sha256"] != content_hash(json.loads(BINARY_SCHEMA_JSON))
            or plan["scope"] != "TRAIN_development_binary_verdicts_not_probabilities"
    ...
    prompt = {"version": BINARY_PROMPT.version, "sha256": BINARY_PROMPT.sha256}
    ...
    for example in examples:
        request = binary_request(example.item, backend.config)
...
async def execute(bundle, audited, *, plan, backend, revision, artifact_root=None,
    ...
    async def run(cap):
        return await run_binary(examples, run_id=plan["run_id"], config=backend.config,
    ...
    with exclusive_run(control):
        ...
        spent = charged_seconds(ledger["windows"], total)
        output = {"study_stage": "post_thesis", "kind": "TRAIN_development_binary_pilot",
                  ...
                  "coverage": report["examples_judged"] / report["examples_total"],
                  "output_contract": "binary_verdict_not_probability", "report": report}
        atomic_json(directory / "binary_pilot_summary.json", output)
...
def main():
    ...
    audited = json.loads((run_directory("ragtruth-train-pilot-50-token-audit-binary-v1") / "audit.json").read_text(encoding="utf-8"))
    record = None
    ...
    print(f"Judged {report['examples_judged']}/{report['examples_total']} TRAIN development examples")
    ...
    print("Private binary pilot summary:", run_directory(plan["run_id"]) / "binary_pilot_summary.json")
    print("Post-thesis development only. Binary verdicts only. No test metrics, threshold fitting or prompt changes.")
    return 0 if report["examples_judged"] == plan["examples"] else 1
