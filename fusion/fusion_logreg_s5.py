...
def extract(nli_map, rel_map, s4_map, s5_map):
    common = sorted(nli_map.keys() & rel_map.keys() & s4_map.keys() & s5_map.keys())
    numeric, categorical, labels = [], [], []
    for idx in common:
        r1, r2, r4, r5 = nli_map[idx], rel_map[idx], s4_map[idx], s5_map[idx]
...
train_num, train_cat, y_train = extract(nli_train, rel_train, s4_train, s5_train)
test_num,  test_cat,  y_test  = extract(nli_test,  rel_test,  s4_test,  s5_test)
...
feature_names = ['NLI score', 'Relevance score', 'Signal4 score', 'BERTScore'] + cat_feature_names
...
print(f"\n--- Logistic Regression Fusion S1+S2+S4+S5 (no S3, threshold={best_threshold:.2f}) ---")
...
results = {
    "method": "Logistic Regression S1+S2+S4+S5 (no S3)",
...
with open('/workspace/fusion_logreg_s5_results.json', 'w') as f:
    json.dump(results, f, indent=2)
print("\nSaved to /workspace/fusion_logreg_s5_results.json")
