...
def extract(rel_map, s4_map):
    ...
    for idx in common:
        ...
        if any(x is None for x in [r2['raw_min_relevance'], r4['signal4_score']]):
            continue
        assert r2['ground_truth_hallucination'] == r4['ground_truth_hallucination']
...
train_num, train_cat, y_train = extract(rel_train, s4_train)
test_num,  test_cat,  y_test  = extract(rel_test,  s4_test)
...
feature_names = ['Relevance score', 'Signal4 score'] + cat_feature_names
...
print(f"\n--- Logistic Regression Fusion S2+S4 only (threshold={best_threshold:.2f}) ---")
...
results = {
    "method": "Logistic Regression S2+S4 only",
...
with open('/workspace/fusion_logreg_s2s4_results.json', 'w') as f:
    json.dump(results, f, indent=2)
print("\nSaved to /workspace/fusion_logreg_s2s4_results.json")
