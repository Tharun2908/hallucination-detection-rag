...
MINICHECK_AUROC_REF = 0.7974


# =============================================================================
# Data loading
# =============================================================================
def _norm_group_text(x):
    if x is None:
        return ""
    return re.sub(r"\s+", " ", str(x)).strip()


def _make_group(source, question, passage):
    text = " ||| ".join([
        _norm_group_text(source),
        _norm_group_text(question),
        _norm_group_text(passage),
    ])
    return hashlib.sha1(
        text.encode("utf-8")
    ).hexdigest()
...
def load_halubench():
    """Load filtered HaluBench with stable canonical grouping."""
...
def group_disjoint_train_val(
    chosen_examples,
    n_val,
    seed,
    search_candidates=5000,
):
    """Find an exact-size, group-disjoint train/validation partition."""

    n_total = len(chosen_examples)
    ...
    train_set = [
        chosen_examples[i]
        for i in best["train_idx"]
    ]

    val_set = [
        chosen_examples[i]
        for i in best["val_idx"]
    ]
    ...
    if overlap:
        raise RuntimeError(
            f"Train/val group leakage: "
            f"{len(overlap)} groups"
        )

    if len(val_set) != n_val:
        raise RuntimeError(
            f"Expected val={n_val}, "
            f"got {len(val_set)}"
        )
...
def sample_train_val(
    ...
    """Balanced adaptation sampling with group-disjoint train/validation."""
    ...
    if n_total > len(
        ...
        raise ValueError(
            f"Requested {n_total} > "
            f"train pool size "
            f"{len(train_pool_examples)}"
        )


    # Preserve original balanced source-label
    # adaptation sampling.
    strata = [
        f"{e['source']}__{e['label']}"
        for e in train_pool_examples
    ]

    chosen_local = (
        balanced_stratified_indices(
            strata,
            n_total,
            seed,
        )
    ...
    chosen_examples = [
        train_pool_examples[i]
        for i in chosen_local
    ]


    train_set, val_set = (
        group_disjoint_train_val(
            chosen_examples,
            n_val,
            seed,
        )
...
def main():
    ...
    # --------------------------------------------------------
    # Canonical group-disjoint outer split.
    #
    # Created after integrity audit:
    #   6000 adaptation-pool examples
    #   8000 fixed-test examples
    #   group = source + normalized question + normalized passage
    #   group overlap = 0
    #   PASS/FAIL paired group overlap = 0
    # --------------------------------------------------------
    ...
    with open(
        ...
        canonical_split = json.load(f)

    train_pool_idx = np.array(
        canonical_split[
    ...
    if (
        len(train_pool_idx) != 6000
        or len(test_idx) != 8000
    ):
        raise RuntimeError(
            "Canonical HaluBench split has "
            "unexpected sizes."
        )
    ...
    if overlap:
        raise RuntimeError(
            f"Outer split leakage: "
            f"{len(overlap)} groups"
        )

    # Save the exact split used with this run,
    # for complete reproducibility.
    with open(
        OUT_DIR / "test_train_pool_indices.json",
        "w",
    ) as f:
        json.dump({
            "train_pool_idx":
    ...
    print(
        "\nLoaded canonical group-disjoint "
        "HaluBench split",
        flush=True,
    )
    ...
    config = {
        ...
        "smoke": args.smoke,
        "notes": (
            "The fixed 8000-example test set is group-disjoint from the "
            "6000-example adaptation pool using source + normalized question "
            "+ normalized passage as the group key. Train sampling remains "
            "balanced by source-label. Inner train/validation partitions are "
            "also group-disjoint. Small-N validation sets remain noisy and "
            "are reported as a limitation."
        ),
    }
