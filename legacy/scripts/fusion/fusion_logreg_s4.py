...
with open('/workspace/consistency_results_test.json') as f:
    con_test = {ex['idx']: ex for ex in json.load(f)}
...
def extract(nli_map, rel_map, con_map, s4_map):
    common = sorted(nli_map.keys() & rel_map.keys() & con_map.keys() & s4_map.keys())
    numeric, categorical, labels = [], [], []
    for idx in common:
        r1, r2, r3, r4 = nli_map[idx], rel_map[idx], con_map[idx], s4_map[idx]
        ...
        numeric.append([
            ...
            r3['consistency_score'],
            r4['signal4_score'],
        ])
        categorical.append([r1['task_type'], r1['model']])
    return np.array(numeric), categorical, np.array(labels)

train_num, train_cat, y_train = extract(nli_train, rel_train, con_train, s4_train)
test_num,  test_cat,  y_test  = extract(nli_test,  rel_test,  con_test,  s4_test)
...
feature_names = ['NLI score', 'Relevance score', 'Consistency score', 'Signal4 score'] + cat_feature_names
...
print(f"\nBest threshold on train: {best_threshold:.2f} (F1={best_f1:.4f})")

# --- Evaluate on test ---
y_prob = clf.predict_proba(X_test)[:, 1]
y_pred = (y_prob >= best_threshold).astype(int)

print(f"\n--- Logistic Regression Fusion with S4 (threshold={best_threshold:.2f}) ---")
...
# --- Save ---
results = {
    "method": "Logistic Regression Fusion S1+S2+S3+S4 (one-hot, train-tuned threshold)",
...
with open('/workspace/fusion_logreg_s4_results.json', 'w') as f:
    json.dump(results, f, indent=2)
print("\nSaved to /workspace/fusion_logreg_s4_results.json")
